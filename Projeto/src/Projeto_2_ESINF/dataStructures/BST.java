...
public class BST<E extends Comparable<E>> implements BSTInterface<E> {
    ...
    protected Node<E> find(Node<E> node, E element) {
        ...
        if (cmp < 0) return find(node.getLeft(), element);
    ...
    /**
     * Wrapper method to access the method find
     *
     * @param element the element to be searched
     * @return the Node in which the Element was found or null if not found
     */
    ...
    public E smallestElement() {
        return this.smallestElement(root);
    }
    ...
    protected E smallestElement(Node<E> node) {
        ...
        if (node.getLeft() != null) {
            return this.smallestElement(node.getLeft());
        } else {
    ...
    private void processBstByLevel(Node<E> node, Map<Integer, List<E>> result, int level) {
        ...
        if (node != null) {
            List<E> elements = result.get(level);

            if (elements == null) {
                elements = new ArrayList<>();
                elements.add(node.getElement());
                result.put(level, elements);
            } else {
                elements.add(node.getElement());
            }
